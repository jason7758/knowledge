...
#### 一、 什么是[负载均衡](https://cloud.tencent.com/product/clb?from=10680)（Load balancing）
...
**四层负载**通过虚拟 IP + 端口接收请求，然后再分配到真实的服务器；

**七层负载**通过虚拟的 URL 或主机名接收请求，然后再分配到真实的服务器。
...
![img](./img/w74mm16wce.jpeg)

负载均衡器通常称为四层交换机或七层交换机。 四层交换机主要分析 IP 层及 TCP/UDP 层，实现四层流量负载均衡。 七层交换机除了支持四层负载均衡以外，还有分析应用层的信息，如 HTTP 协议 URI 或 Cookie 信息。

负载均衡分为 L4 Switch（四层交换），即在 OSI 第 4 层工作，就是 TCP 层啦。 此种 Load Balancer 不理解应用协议（如 HTTP/FTP/[MySQL](https://cloud.tencent.com/product/cdb?from=10680) 等等）。例子：LVS，F5。
